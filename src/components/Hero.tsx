...
const SearchForm = () => {
  const [query, setQuery] = useState('graphic designer');
  ...
  return (
    ...
    >
      <div
        id="form-control"
        className="flex items-center bg-white rounded-full group-focus-within:ring-2 group-focus-within:ring-indigo-400 group-focus-within:ring-offset-2 group-focus-within:ring-offset-transparent overflow-hidden"
      >
        ...
        <input
          ...
          onFocus={() => !isDirty && setQuery('')}
          onBlur={() => (query !== '' ? setIsDirty(true) : setIsDirty(false))}
          value={query}
